...
- [ ] **Phase 1: Backend Agent Spine + Data Architecture** - Promote `aiChiefOfStaffService` into a dedicated `agent/` module with tool registry, action ledger, memory store, and the 9 Phase-1 read tools, all tenant-scoped; add the five new Prisma models (AgentAction, AgentMemory, PinnedView, PerformanceSnapshot, MetricEvent).
