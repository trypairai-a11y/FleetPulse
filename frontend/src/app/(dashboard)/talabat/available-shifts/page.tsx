...
const API_BASE = "/api/talabat/available-shifts";
...
export default function TalabatAvailableShiftsPage() {
  ...
  const { data, loading, error, refetch } = useApiGet<SlotResponse>(`${API_BASE}?${qs.toString()}`);
    ...
    <div className="space-y-6">
      <header className="flex items-start justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-secondary mt-1">Open shift slots from the Talabat driver app — capacity vs claimed, by area.</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setCreating(true)} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg bg-talabat text-white text-sm hover:opacity-90">
      ...
      {!loading && byArea.length === 0 && (
        <div className="bg-white border border-gray-100 rounded-2xl p-10 text-center">
          ...
          <button onClick={() => setCreating(true)} className="mt-3 inline-flex items-center gap-1.5 px-3 py-2 rounded-lg bg-talabat text-white text-sm">
        ...
        {byArea.map((group) => (
          <div key={group.area} className="bg-white border border-gray-100 rounded-2xl overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100">
              <div className="flex items-center gap-2">
                <MapPin size={16} className="text-talabat" />
...
function SlotForm({ initial, onSaved }: { initial: any; onSaved: () => void }) {
  ...
  async function save() {
    ...
    try {
      ...
      if (form.id) await api.put(`${API_BASE}/${form.id}`, payload);
  ...
  return (
    <div className="space-y-4 p-5">
      ...
      <div className="flex justify-end gap-2 pt-2">
        <button onClick={save} disabled={saving} className="px-4 py-2 rounded-lg bg-talabat text-white text-sm disabled:opacity-50">
