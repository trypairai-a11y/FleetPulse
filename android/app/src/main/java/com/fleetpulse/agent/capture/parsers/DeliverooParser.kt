...
object DeliverooParser : PlatformParser {

    private val ORDER_REF_PATTERN = Regex("""#(\w+[-]?\d+)""")
    private val AMOUNT_PATTERN = Regex("""([\d.]+)\s*(?:KWD|د\.ك|KD)""", RegexOption.IGNORE_CASE)
    private val ASSIGNED_KEYWORDS = listOf("new order", "طلب جديد", "order available", "طلب متاح")
    private val DELIVERED_KEYWORDS = listOf("delivered", "تم التوصيل", "completed", "مكتمل")
    private val PICKED_KEYWORDS = listOf("picked", "تم الاستلام", "collected", "تم التجميع")
